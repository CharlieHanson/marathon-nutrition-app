...
/**
 * Maximum allowed calorie overshoot after scaling (5%).
 * If final calories exceed target * (1 + CALORIE_CEILING_PCT), all
 * ingredients are proportionally scaled down to fit.
 */
const CALORIE_CEILING_PCT = 0.05;
 ...
 *   2. Scale carb-type ingredients to hit carb target (accounting for protein's carb contribution)
 *   3. Scale fat-type ingredients to hit fat target (accounting for protein + carb fat contribution)
 *      — NOW SCALES DOWN when other sources already exceed fat budget
 *   4. Leave vegetable portions unchanged (low caloric impact, mainly volume)
 *   5. Calorie ceiling: if total calories still exceed budget by >5%, proportionally
 *      scale ALL ingredients down to fit
 */
export function adjustPortions(ingredients, target) {
  ...
  // FIX: Scale fat sources both UP and DOWN
  // Old code only scaled up (fatNeededFromFat > 0), leaving fat unscaled
  // when protein/carb sources already exceeded the fat budget
  if (fatFromFat > 0) {
    if (fatNeededFromFat > 0) {
      // Need more fat — scale up
      const fatScale = fatNeededFromFat / fatFromFat;
      scaleType('fat', fatScale);
    } else if (fatNeededFromFat < 0) {
      // Other sources already exceed fat budget — scale fat sources DOWN
      // Try to minimize fat to its floor (scale toward 0, clamped to MIN_SCALE)
      const fatScale = Math.max(0, fatNeededFromFat / fatFromFat);
      scaleType('fat', fatScale);
    }
  }

  // Step 4: CALORIE CEILING — prevent total calorie overshoot
  // After independently scaling P/C/F, the combined calories can exceed budget.
  // If over by >5%, proportionally shrink ALL ingredients to fit.
  const postScaleMacros = computeMacros(adjusted);
  const calorieLimit = target.calories * (1 + CALORIE_CEILING_PCT);

  if (postScaleMacros.calories > calorieLimit) {
    const calorieFactor = target.calories / postScaleMacros.calories;
    console.log(
      `⚠️ Calorie ceiling hit: ${postScaleMacros.calories} > ${Math.round(calorieLimit)}. ` +
      `Scaling all ingredients by ${(calorieFactor * 100).toFixed(0)}%`
    );

    adjusted = adjusted.map((ing) => {
      const type = (ing.type || '').toLowerCase();
      const bounds = PORTION_BOUNDS[type] || { min: 0, max: 500 };
      const newGrams = Math.round(ing.grams * calorieFactor);
      return {
        ...ing,
        grams: Math.max(bounds.min, Math.min(bounds.max, newGrams)),
      };
    ...
    scaleFactors._calorieCeiling = Math.round(calorieFactor * 100) / 100;
  }
 ...
 *
 * @param {Array<{ name, type, grams }>} ingredients - from AI response
